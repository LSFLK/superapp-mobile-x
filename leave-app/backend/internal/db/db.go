...
func (db *Database) Migrate() error {
    ...
    migrations := []string{
        ...
        "migrations/005_update_half_day_period.sql",
...
func CalculateWorkingDays(start, end time.Time, holidays map[string]bool) []time.Time {
    var days []time.Time
...
func (db *Database) GetAllLeaves() ([]models.Leave, error) {
    ...
    for rows.Next() {
        ...
        // Populate days for each leave
        days, err := db.GetLeaveDays(leave.ID)
        if err != nil {
            return nil, fmt.Errorf("failed to get leave days for leave %s: %w", leave.ID, err)
        }
        leave.Days = days
...
func (db *Database) CreateLeaveDays(leaveID string, dates []time.Time, isHalfDay bool, halfDayPeriod *string) error {
...
func (db *Database) GetHolidays() (map[string]bool, error) {
    rows, err := db.Conn.Query("SELECT date FROM holidays")
    if err != nil {
        return nil, err
    }
    defer rows.Close()
...
func (db *Database) GetAllHolidays() ([]models.Holiday, error) {
    rows, err := db.Conn.Query("SELECT id, date, name FROM holidays ORDER BY date")
    if err != nil {
        return nil, err
    }
    defer rows.Close()
...
func (db *Database) GetLeaveDays(leaveID string) ([]models.LeaveDay, error) {
    query := "SELECT id, leave_id, date, is_half_day, half_day_period FROM leave_days WHERE leave_id = ? ORDER BY date"
    rows, err := db.Conn.Query(query, leaveID)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
...
// GetLeaveDayByID returns a specific leave day by ID
func (db *Database) GetLeaveDayByID(dayID string) (*models.LeaveDay, error) {
    query := "SELECT id, leave_id, date, is_half_day, half_day_period FROM leave_days WHERE id = ?"
    var day models.LeaveDay
    var dateTime time.Time
    err := db.Conn.QueryRow(query, dayID).Scan(&day.ID, &day.LeaveID, &dateTime, &day.IsHalfDay, &day.HalfDayPeriod)
    if err != nil {
        return nil, err
    }
    day.Date = dateTime.Format("2006-01-02")
...
// UpdateSingleDayLeaveHalfDay updates the half-day status for a single-day leave
func (db *Database) UpdateSingleDayLeaveHalfDay(leaveID string, isHalfDay bool, halfDayPeriod *string) error {
    query := "UPDATE leave_days SET is_half_day = ?, half_day_period = ? WHERE leave_id = ?"
    _, err := db.Conn.Exec(query, isHalfDay, halfDayPeriod, leaveID)
    return err
}

// ReplaceLeaveDaysAndUpdateLeave replaces all leave day records for a leave and updates the leave
func (db *Database) ReplaceLeaveDaysAndUpdateLeave(leaveID, startDate, endDate string, totalDays float64, days []time.Time) error {
    if len(days) == 0 {
