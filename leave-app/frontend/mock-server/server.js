...
let users = [
  { id: 'u1', email: 'alice@example.com', role: 'admin', allowances: { sick: 10, annual: 14, casual: 7 } },
  { id: 'u2', email: 'bob@example.com', role: 'user', allowances: { sick: 5, annual: 10, casual: 5 } }
];

let leaves = [
  {
    id: 'L1', userId: 'u2', userEmail: 'bob@example.com', type: 'annual',
    startDate: new Date(Date.now() - 1000 * 60 * 60 * 24 * 10).toISOString().slice(0,10),
    endDate: new Date(Date.now() - 1000 * 60 * 60 * 8).toISOString().slice(0,10),
    reason: 'Vacation', status: 'approved', createdAt: new Date().toISOString()
  }
];
...
const server = http.createServer(async (req, res) => {
  ...
  console.log(`[mock-server] ${req.method} ${url.pathname} - Host: ${req.headers.host}`);
  ...
  try {
    ...
    const partsDebug = url.pathname.replace(/^\/api\/?/, '').split('/').filter(Boolean);
    ...
    if (parts.length === 2 && parts[0] === 'admin' && parts[1] === 'allowances' && req.method === 'PUT') {
      if (!authOK(req)) return sendJSON(res, 401, { error: 'Unauthorized' });
    ...
    if (parts[0] === 'leaves') {
      ...
      if (id && parts.length >= 2) {
        ...
        if (parts.length === 3 && req.method === 'POST' && parts[2] === 'approve') {
          if (!authOK(req)) return sendJSON(res, 401, { error: 'Unauthorized' });
          if (idx === -1) return sendJSON(res, 404, { error: 'Not found' });
          const body = await parseBody(req);
        ...
        if (parts.length === 3 && req.method === 'POST' && parts[2] === 'reject') {
          if (!authOK(req)) return sendJSON(res, 401, { error: 'Unauthorized' });
          if (idx === -1) return sendJSON(res, 404, { error: 'Not found' });
          const body = await parseBody(req);
