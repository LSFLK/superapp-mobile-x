...
const BalanceCard = ({ type, remaining, total }: { type: string, remaining: number, total: number }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 p-3 shadow-sm flex flex-col justify-between">
      ...
      <div className="mt-2">
        <span className={`text-xl font-bold ${isLow ? 'text-red-500' : 'text-slate-800'}`}>{remaining}</span>
...
export const MyLeaves: React.FC<MyLeavesProps> = ({ leaves, balances, onDelete, onRequestNew, filters }) => {
  ...
  return (
    <div className="space-y-6 pb-24">
      <section>
        <h2 className="text-xs font-bold uppercase text-slate-400 mb-3 px-1 tracking-wide">Your Balances</h2>
        <div className="grid grid-cols-3 gap-3">
          <BalanceCard type="Annual" remaining={balances.annual} total={balances.total.annual} />
          <BalanceCard type="Sick" remaining={balances.sick} total={balances.total.sick} />
          <BalanceCard type="Casual" remaining={balances.casual} total={balances.total.casual} />
        </div>
      </section>
      ...
      <div className="border-t border-slate-200 pt-4">
        ...
        {leaves.length === 0 ? (
          ...
          <div className="space-y-4 mt-4">
            {leaves.map((leave) => (
              <Card key={leave.id} className="relative overflow-hidden group transition-all hover:shadow-md">
                ...
                <div className="flex items-center text-xs text-slate-500 space-x-4">
                  ...
                  <div className="flex items-center">
                      ...
                      <span>{formatDuration(leave.startDate, leave.endDate)} days</span>
                  </div>
                </div>
                ...
                {leave.status === 'rejected' && leave.approverComment && (
                   <div className="mt-3 bg-red-50 p-2 rounded-lg text-xs text-red-700 border border-red-100 flex items-start">
                      ...
                      <span><span className="font-semibold">Rejected:</span> {leave.approverComment}</span>
                   </div>
                )}
              </Card>
            ))}
