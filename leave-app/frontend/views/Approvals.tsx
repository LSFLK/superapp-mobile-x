...
export const Approvals: React.FC<ApprovalsProps> = ({ leaves, onApprove, onReject }) => {
  const [rejectModalOpen, setRejectModalOpen] = useState(false);
  const [selectedLeaveId, setSelectedLeaveId] = useState<string | null>(null);
  const [rejectReason, setRejectReason] = useState('');
  const { user } = useAuth();
  const pendingLeaves = leaves.filter(l => l.status === 'pending' && l.userEmail !== user?.email);
        ...
        {pendingLeaves.map((leave) => (
          <Card key={leave.id}>
            <div className="flex items-center space-x-3 mb-3">
              ...
              <div>
                  <p className="text-sm font-semibold text-slate-900">{leave.userEmail}</p>
                  <p className="text-xs text-slate-500">{formatDate(leave.createdAt)}</p>
              </div>
            ...
            <div className="bg-slate-50 rounded-xl p-3 mb-4 border border-slate-100">
              <p className="text-sm font-medium text-slate-800 mb-1">{leave.reason}</p>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-slate-500">
            Are you sure you want to reject this leave request? You can optionally provide a reason below.
          </p>
          <div>
            <label className="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wide">Reason (Optional)</label>
            <textarea
